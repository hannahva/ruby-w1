...
def grab_sin(string)
  sin_match = /\d{3}-\d{3}-\d{3}/.match(string)
  return sin_match.to_s unless sin_match.nil? == true
end
...
def grab_all_sins(string)
  sins_array = string.scan(/\d{3}-\d{3}-\d{3}/)
  sins_array
end
...
def format_sins(string)
  if string.scan(/(?:^|\D)\d{3}-\d{3}-\d{3}$/) != []
    string.gsub(/\./, '').gsub(/-/, '').gsub(/(\d{3})(?=\d)/, '\1-\2')
  else
    string
  end
end
